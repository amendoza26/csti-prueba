...
export default class CardRoutes extends BaseRouter {
  ...
  protected handler(router: Router): void {
    this.get({
      router,
      route: ControllerAdapter.handle(
        container.get("Apps.controllers.GetCardController")
      ),
      middlewares: [
        MiddlewareAdapter.handle(
          container.get("Apps.middlewares.CheckTokenHeaderMiddleware")
        ),
      ],
      url: "/",
    });
