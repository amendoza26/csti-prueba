...
export class CreateToken extends BaseUseCase<Request, any> {
  ...
  async run(request: Request): Promise<UseCaseResponse> {
    ...
    const token = sign(request, `${Configuration.JWT_KEY}`, {
      expiresIn: "1m",
    });
  ...
  private validateLength(request: Record<string, any>): boolean {
    if (
      request.card_number.toString().length < 13 ||
      request.card_number.toString().length > 16
    ) {
      return false;
    }
    if (
      request.cvv.toString().length < 3 ||
      request.cvv.toString().length > 4
    ) {
      return false;
    }
